...
namespace PFR2_Floreria
{
    public partial class Tipo_Venta : Form
    {
        ...
        {
            DataGridView1.DataSource = Conexionn.Query("Select * from Tipo_Venta order by id");
        }

        private void BtnAgregarTipoArreglo_Click(object sender, EventArgs e)
        {
            Conexionn.conexionn.Open();
            NpgsqlCommand xd = new NpgsqlCommand("Insert into Tipo_Venta(descripcion,empleado_crea,empleado_actualiza) values (@descripcion,@empleado_crea,@empleado_actualiza)", Conexionn.conexionn);
        ...
        {
            Conexionn.conexionn.Open();
            NpgsqlCommand xd = new NpgsqlCommand("Update Tipo_Venta set descripcion=@descripcion,empleado_crea=@empleado_crea,empleado_actualiza=@empleado_actualiza where id = @id", Conexionn.conexionn);
        ...
        {
            Conexionn.conexionn.Open();
            NpgsqlCommand xd = new NpgsqlCommand("Update Tipo_Venta set status=0 where id = @id", Conexionn.conexionn);
